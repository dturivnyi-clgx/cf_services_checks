        ...
        if (mysqlService == null)
        {
            ...
            if (mysqlService != null) {
                ...
                if (mysqlService != null) {
                    ...
                    mysqlService = (List) jsonMap.get("a9s-mysql104");
                    if (mysqlService != null) {
                        log.info("Obtaining VCAP_SERVICES credentials - a9s-mysql104");
                        cfMySQLMap = (Map) mysqlService.get(0);
                        credentailsMap = (Map) cfMySQLMap.get("credentials");

                        login.setUrl("jdbc:mysql://" + (String) credentailsMap.get("host") + ":3306/" + (String) credentailsMap.get("name"));
